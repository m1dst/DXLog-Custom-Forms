...
namespace System.Windows.Forms
{
    ...
    public partial class MapControl : Control
    {
        ...
        {
            var drawLayerList = new List<Layer>(Layers).OrderBy(l => l.Level).ToList();
            foreach (var layer in drawLayerList)
            {
        ...
        private void DrawSingleLayer(Layer layer, Graphics gr)
        {
            if (layer.Visible)
            {
                switch (layer)
                {
                    ...
                    {
                        foreach (var track in trackLayer.Tracks)
                        {
                            DrawSingleTrack(track, gr);
                        }
                        break;
                    }
                    case PolygonLayer polygonLayer:
                    {
                        foreach (var polygon in polygonLayer.Polygons)
                        {
                            DrawSinglePolygon(polygon, gr);
                        }
                        break;
                    }
                    case EllipseLayer ellipseLayer:
                    {
                        foreach (var ellipse in ellipseLayer.Ellipses)
                        {
        ...
        private void DrawSingleMarker(Marker marker, Graphics gr)
        {
            ...
            if (marker.Style.MarkerWidth == 0 || (p.X > -marker.Style.MarkerWidth && p.X < this.ClientSize.Width + marker.Style.MarkerWidth && p.Y > -marker.Style.MarkerWidth && p.Y < this.ClientSize.Height + marker.Style.MarkerWidth))
            {
                Draw(gr, () =>
                {
